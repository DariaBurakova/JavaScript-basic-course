...
function handleClick(clickedButtonEvent) {
    ...
    let textBut = card.button.innerText
    
    if (textBut == "Подробнее" ) { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (textBut == "Отмена" ) { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let text = 'Здесь должен быть текст';
